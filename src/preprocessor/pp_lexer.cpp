...
void pp_lexer::tokenize_pp_number(pp_lexer_results &results)
{
    ...
    add_token(results, pp_token_type::pp_number);
...
void pp_lexer::tokenize_pp_char_constant(pp_lexer_results &results)
{
    ...
    add_token(results, pp_token_type::pp_char_constant);

    //    results.tokens.emplace_back(m_source.substr(m_suboffset, m_pos - m_suboffset + 1),
    //        pp_token_type::pp_char_constant, m_line_offset - (m_pos - m_suboffset),
    //        first_in_line);
}
...
void pp_lexer::tokenize_pp_string_literal(pp_lexer_results &results)
{
    ...
    add_token(results, pp_token_type::pp_string_literal);

    //    results.tokens.emplace_back(m_source.substr(m_suboffset, m_pos - m_suboffset + 1),
    //        pp_token_type::pp_string_literal, m_line_offset - (m_pos - m_suboffset),
    //        first_in_line);
}
...
void pp_lexer::tokenize_pp_header_name(pp_lexer_results &results)
{
    ...
    add_token(results, pp_token_type::pp_header_name);
...
void pp_lexer::tokenize_line(pp_lexer_results &result)
{
    for (char current = m_source[m_pos]; !is_end(); next()) {
        ...
        switch (current) {
            ...
            case '#':
                if (peek_next() == '#') {
                    result.tokens.emplace_back(
                        "##", pp_token_type::p_double_hash, m_line_offset, first_in_line);
                    next();
                } else
                    result.tokens.emplace_back(
            ...
            case '(':
                result.tokens.emplace_back(
                    "(", pp_token_type::p_leftparentheses, m_line_offset, first_in_line);
                break;

            case ')':
                result.tokens.emplace_back(
                    ")", pp_token_type::p_rightparentheses, m_line_offset, first_in_line);
                break;

            case ',':
                result.tokens.emplace_back(
                    ",", pp_token_type::p_comma, m_line_offset, first_in_line);
                break;

            case '.':
                if (peek_next() == '.' && peek(2) == '.') {
                    result.tokens.emplace_back(
                        "...", pp_token_type::p_tripledot, m_line_offset, first_in_line);
                    next();
                    next();
                } else
                    result.tokens.emplace_back(
                        ".", pp_token_type::p_dot, m_line_offset, first_in_line);
                break;
...
std::string tokens_to_text(std::vector<pp_token> &tokens)
{
    ...
    for (size_t i = 0; i < tokens.size(); i++) {
        ...
        if (token.first_in_line) {
            if (i > 0)
                result += "\n"; // do not add new line at the beginning
