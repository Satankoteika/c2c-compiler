...
namespace ctc::lexer
{
    ...
    static std::unordered_map<std::string, token_type> keywords_token_table{
        {"auto", token_type::k_auto},       {"break", token_type::k_break},

        {"case", token_type::k_case},       {"char", token_type::k_char},

        {"const", token_type::k_const},     {"continue", token_type::k_continue},
        {"default", token_type::k_default},

        {"do", token_type::k_do},           {"double", token_type::k_double},

        {"else", token_type::k_else},       {"enum", token_type::k_enum},
        {"extern", token_type::k_extern},

        {"float", token_type::k_float},     {"for", token_type::k_for},
        {"goto", token_type::k_goto},

        {"if", token_type::k_if},           {"inline", token_type::k_inline},
        {"int", token_type::k_int},         {"long", token_type::k_long},

        {"return", token_type::k_return},   {"short", token_type::k_short},
        {"signed", token_type::k_signed},   {"sizeof", token_type::k_sizeof},

        {"static", token_type::k_static},   {"struct", token_type::k_struct},
        {"switch", token_type::k_switch},   {"typedef", token_type::k_typedef},

        {"union", token_type::k_union},     {"unsigned", token_type::k_unsigned},
        {"void", token_type::k_void},       {"volatile", token_type::k_volatile},

        {"while", token_type::k_while},     {"bool", token_type::k_bool}};
    ...
    lexer_results lexer::tokenize_from_source(std::string &&source)
    {
        ...
        while (!is_end())
        {
            ...
            switch (character)
            {
                ...
                // Punctuators
                case '[':
                    results.tokens.emplace_back(token_type::p_leftbracket, "empty");
                    break;
                case ']':
                    results.tokens.emplace_back(token_type::p_rightbracket, "empty");
                    break;

                case '(':
                    results.tokens.emplace_back(token_type::p_leftparentheses, "empty");
                    break;
                case ')':
                    results.tokens.emplace_back(token_type::p_rightparentheses, "empty");
                    break;

                case '{':
                    results.tokens.emplace_back(token_type::p_leftbrace, "empty");
                    break;
                case '}':
                    results.tokens.emplace_back(token_type::p_rightbrace, "empty");
                    break;
                ...
                case '~':
                    results.tokens.emplace_back(token_type::p_tilde, "empty");
                    break;
                ...
                case '?':
                    results.tokens.emplace_back(token_type::p_question, "empty");
                    break;

                case ':':
                    results.tokens.emplace_back(token_type::p_colon, "empty");
                    break;
                case ';':
                    results.tokens.emplace_back(token_type::p_semicolon, "empty");
                    break;

                case ',':
                    results.tokens.emplace_back(token_type::p_comma, "empty");
                    break;
    ...
    void lexer::tokenize_string_literal(lexer_results &results) noexcept
    {
        ...
        results.tokens.emplace_back(token_type::l_string_literal,
                                    m_source.substr(m_suboffset, m_pos - m_suboffset));
    }

    void lexer::tokenize_number_literal(lexer_results &results)
    {
        number_literal_format format;
        ...
        if (m_source[m_pos] == '0')
        {
            ...
            {
                case 'b':
                    format = number_literal_format::Bin;
                    break;
                case 'x':
                    format = number_literal_format::Hex;
                    break;
            }
        }
        ...
        if (m_source[m_pos] == '.' && !is_digit(peek_next()))
        {
            results.errors.emplace_back(
                lexer_error{"Unable to process fractional literal", m_line, m_pos});
            return;
        }
        ...
        results.tokens.emplace_back(token_type::l_number_literal,
                                    m_source.substr(m_suboffset, m_pos - m_suboffset));
    }
    ...
    void lexer::tokenize_identifier(lexer_results &results)
    {
        ...
        results.tokens.emplace_back((keywords_token_table.contains(literal_value)
                                         ? keywords_token_table.at(literal_value)
